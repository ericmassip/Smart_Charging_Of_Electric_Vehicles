...
def split_T_reg(iteration, F, previous_Q_approximated_function):
    x = []
    y = []
...
# Training
for iteration in range(1, Smax + 1):
    print('')
    print('')
    print('Iteration number ' + str(iteration))

    x_T_reg, y_T_reg = split_T_reg(iteration, train_F, previous_Q_approximated_function)
...
for iteration in range(1, Smax + 1):
    ...
    model = load_model(models_directory + 'Q' + str(iteration) + '_approximated_function.h5')
