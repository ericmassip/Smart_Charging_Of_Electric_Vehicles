...
def split_T_reg(F, previous_Q_approximated_function):
    ...
    for i in range(len(F)):
        ...
        x.append([s_a_tuple.pv, *s_a_tuple.Xs.flatten(), *s_a_tuple.us])
...
def predict(s_a_tuple, action, previous_Q_approximated_function):
    predict_me_sth = np.array([s_a_tuple.next_pv, *s_a_tuple.resulting_Xs.flatten(), *action])
...
# plus a vector of size Smax plus 1 for the pv
...
for timeslot in range(1, Smax + 1):
    ...
    predict_me_sth = np.array(np.array([s_a_tuple.next_pv, *s_a_tuple.Xs.flatten(), *s_a_tuple.us]))
